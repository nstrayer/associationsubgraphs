...
data.forEach((d, i) => {
  ...
  if (n_mergers > 0) {
    ...
    max_merger_magnitude = Math.max(
      max_merger_magnitude,
      total_merger_magnitude
    );
  }
...
const x = d3.scaleLinear().domain([1, steps.length]).range([0, w]);
...
const series = d3
  ...
  .order(d3.stackOrderInsideOut)(steps);

// Build scales
charts.stream.y.domain([
...
charts.n_in_subgraphs.y.domain([0, total_num_nodes]);
...
charts.mergers.g
  ...
  .attr(
    "y",
    (d) => charts.mergers.height / 2 - charts.mergers.y(d.magnitude) / 2
  )
...
charts.n_subgraphs.y.domain([0, max_num_subgraphs]).nice();
...
const max_step = num_subgraphs_at_step.findIndex(
  ({ step, n }) => n === max_num_subgraphs
);
...
const place_callouts = function (step_i) {
  ...
  n_in_subgraphs_callout.update({
    ...
    txt: `${nodes_in_subgraphs} nodes (${d3.format(".1%")(
      nodes_in_subgraphs / total_num_nodes
    )}) in subgraphs`,
  });
...
function extend_ticks(g, tick_width, tick_opacity = 0.1) {
  ...
  g.select(`.tick:first-of-type line`).attr("stroke-opacity", 0.7);

  g.selectAll(`.tick text`).attr("y", -outcrop).attr("x", 0);
}
...
function make_callout(g) {
  ...
  const background_rect = callout_g
    .append("rect")
    .attr("fill", "white")
