...
virus_host_net <- read_csv(here("data-raw/mammal_virus_associations.csv")) %>%
  select(
    virus_id = vVirusNameCorrected,
    host_id = hHostNameFinal,
...
usethis::use_data(virus_host_net, overwrite = TRUE)
usethis::use_data(virus_host_net_hosts, overwrite = TRUE)
usethis::use_data(virus_host_net_viruses, overwrite = TRUE)
